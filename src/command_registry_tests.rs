...
const GOLDEN_KNOWN: &[&str] = &[
    "git", "ls", "lsd", "exa", "eza", "tree", "find", "fd", "grep", "rg", "ag",
    "ack", "tail", "cargo", "npm", "pnpm", "bun", "yarn", "pip", "pip3",
    "pytest", "jest", "vitest", "make", "cmake", "tsc", "gcc", "g++", "clang",
    "javac", "docker", "gh", "env", "printenv", "wc", "wget", "curl", "eslint",
    "biome", "ruff", "pylint", "golangci-lint", "ollama", "kubectl", "swift",
    "xcodebuild", "ping", "brew", "python", "python3", "npx", "ps", "uv", "trs",
    "cat", "head", "sed", "cd", "echo", "go", "poetry",
];
...
fn rewrite_eligibility_matches_legacy_prefixes() {
    ...
    for cmd in [
        "git", "ls", "lsd", "exa", "eza", "tree", "find", "fd", "grep", "rg",
        "ag", "ack", "tail", "cargo", "npm", "pnpm", "bun", "yarn", "pip",
        "pip3", "pytest", "jest", "vitest", "make", "cmake", "tsc", "gcc",
        "g++", "clang", "javac", "docker", "gh", "env", "printenv", "wc",
        "wget", "curl", "eslint", "biome", "ruff", "pylint", "golangci-lint",
        "ollama", "kubectl", "swift", "xcodebuild", "ping", "brew", "python",
        "python3", "npx", "ps", "uv", "bash", "node", "awk", "du", "jq",
    ] {
    ...
    for cmd in ["go", "poetry", "psql", "journalctl", "cat", "cd"] {
        assert!(!is_rewrite_command(cmd), "{cmd} should not be in explicit set");
    }
}
