...
impl ReadHandler {
    pub(crate) fn execute(&self, input: &ReadInput, ctx: &CommandContext) -> CommandResult {
        let content = std::fs::read_to_string(&input.file).map_err(|e| {
            CommandError::IoError(format!("{}: {}", input.file.display(), e))
        })?;
...
fn detect_language(path: &PathBuf) -> Language {
    ...
    match ext.to_lowercase().as_str() {
        ...
        | "lock" | "svg" | "html" | "htm" => Language::Data,
        ...
        "js" | "jsx" | "ts" | "tsx" | "mjs" | "cjs" | "mts" | "cts" | "vue" | "svelte" => Language::JavaScript,
        // Go
        "go" => Language::Go,
        // Java-like (C-style comments)
        "java" | "c" | "h" | "cpp" | "hpp" | "cc" | "cxx" | "cs" | "kt" | "kts"
        | "swift" | "scala" | "groovy" | "dart" | "m" | "mm" => Language::Java,
        // Ruby
...
fn is_comment_line(line: &str, lang: Language) -> bool {
    ...
    let important = ["TODO", "FIXME", "HACK", "XXX", "SAFETY", "IMPORTANT",
                     "type:ignore", "noqa", "pragma", "eslint-disable", "ts-ignore",
                     "@ts-expect-error", "nolint", "allow(", "deny("];
...
fn filter_aggressive(content: &str, lang: Language) -> String {
    ...
    for line in content.lines() {
        ...
        // Skip block comments
        if !in_block_comment && (trimmed.starts_with("/*") || trimmed.starts_with("/**")) {
            if matches!(lang, Language::Rust | Language::Go | Language::Java | Language::JavaScript) {
                if !trimmed.contains("*/") {
                    in_block_comment = true;
                }
                continue;
        ...
        if in_body && matches!(lang, Language::Rust | Language::Go | Language::Java | Language::JavaScript) {
            brace_depth += count_braces(trimmed);
...
fn is_import_line(trimmed: &str, lang: Language) -> bool {
    match lang {
        Language::Rust => trimmed.starts_with("use ") || trimmed.starts_with("pub use ")
            || trimmed.starts_with("extern crate ") || trimmed.starts_with("mod ")
        ...
        Language::JavaScript => trimmed.starts_with("import ") || trimmed.starts_with("export ")
...
fn is_definition_line(trimmed: &str, lang: Language) -> bool {
    match lang {
        ...
        Language::JavaScript => {
            ...
            || trimmed.starts_with("export function ") || trimmed.starts_with("export default function ")
            || trimmed.starts_with("export class ") || trimmed.starts_with("export default class ")
        ...
        Language::Java => {
            // public void method() { ... }
            let has_access = trimmed.starts_with("public ") || trimmed.starts_with("private ")
                || trimmed.starts_with("protected ") || trimmed.starts_with("static ")
                || trimmed.starts_with("abstract ") || trimmed.starts_with("final ");
            let has_def = trimmed.contains("class ") || trimmed.contains("interface ")
                || trimmed.contains("enum ")
                || (trimmed.contains("(") && (trimmed.contains(") {") || trimmed.contains(") throws")));
            has_access && has_def
        }
        Language::Ruby => {
...
fn apply_line_range<'a>(lines: &'a [&str], max_lines: Option<usize>, tail_lines: Option<usize>) -> Vec<&'a str> {
