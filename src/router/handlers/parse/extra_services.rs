...
impl ParseHandler {
    ...
    pub(crate) fn handle_gh_run_view(
        ...
        for (raw_line, clean_line) in raw_input.lines().zip(input.lines()) {
            ...
            // First content line: extract run title and conclusion
            if title.is_empty() {
                // Strip leading status char (first word may be emoji residue or single char)
                let line_clean = trimmed.trim_start_matches(|c: char| !c.is_alphanumeric());
                // Extract the run/workflow name — take text after last ": " or use whole line
