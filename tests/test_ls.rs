//! Comprehensive integration tests for the `ls` parse command.
//!
//! This test module verifies ls output parsing functionality through the CLI:
//! - Basic ls parsing (simple, long format, with directories, hidden files)
//! - Output format variations (JSON, CSV, TSV, Agent, Raw, Compact)
//! - Stats output including raw vs reduced output size comparison
//! - Edge cases (empty output, special characters, errors)
...
fn test_ls_stats_with_file_flag() {
    ...
    let output = cmd
        ...
        .arg("ls")
        .arg("--file")
        .arg("tests/fixtures/ls_simple.txt")
        .assert()
        .success()
        .get_output()
        .clone();
